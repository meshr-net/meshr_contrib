...
static int nixio_glob__iter(lua_State *L) {
   nixio_glob_t *globres = lua_touserdata(L, lua_upvalueindex(1));
    printf("%s:%d \n", __func__, __LINE__); 
   if (globres->gl.gl_pathv && !globres->freed && globres->pos < globres->gl.gl_pathc) {
      printf("%s:%d %d %d %p\n", __func__, __LINE__,globres->pos, globres->gl.gl_pathc, globres->gl.gl_pathv + globres->pos + 1); 
      int p = (globres->pos)++;
      lua_pushstring(L, globres->gl.gl_pathv[p]);
   } else {
      if (!globres->freed) {
         globfree(&globres->gl);
         globres->freed = 1;
...
static int nixio_glob(lua_State *L) {
    ...
    printf("%s:%d %s %d\n", __func__, __LINE__, pattern, globres->gl.gl_pathc); 
    }
    return 2;
}
